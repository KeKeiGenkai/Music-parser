...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .folder summary { font-weight: 600; margin-bottom: 0.5rem; color: #58a6ff; display: flex; align-items: center; gap: 0.75rem; cursor: pointer; list-style: none; padding: 0.25rem 0; }
        .folder summary::-webkit-details-marker { display: none; }
        .folder summary::before { content: '▶ '; font-size: 0.7rem; margin-right: 0.3rem; }
        .folder[open] summary::before { content: '▼ '; }
        .folder-content { padding-left: 1.2rem; margin-top: 0.3rem; }
    ...
    <script>
        ...
        async function loadRecordings() {
            ...
            for (const f of d.folders) {
                html += '<details class="folder"><summary>' + escapeHtml(f.name);
                if (f.tracks.length > 0) {
                    html += '<a href="/api/download-folder/' + encodeURIComponent(f.name) + '" class="dl-all" download onclick="event.stopPropagation()">Скачать всё ZIP</a>';
                }
                html += '</summary><div class="folder-content">';
